// Test script to verify admin routing fix
const http = require('http');

console.log('Testing admin routing fix...');

// Test 1: Check if /admin/login serves HTML content
const test1 = () => {
  return new Promise((resolve) => {
    const req = http.get('http://localhost:3000/admin/login', (res) => {
      ...
      res.on('end', () => {
        ...
        console.log('Test 1 - /admin/login serves HTML:', isHtml ? 'PASS' : 'FAIL');
        if (!isHtml) {
    ...
    req.on('error', (err) => {
      console.log('Test 1 - /admin/login:', 'FAIL -', err.message);
      resolve(false);
    });
    req.setTimeout(5000, () => {
      console.log('Test 1 - /admin/login:', 'FAIL - Timeout');
      req.destroy();
      resolve(false);
    });
  });
...
const runTests = async () => {
  console.log('Running admin routing tests...\n');
  ...
  try {
    ...
    console.log('\n--- Test Results ---');
    console.log('Test 1 (/admin/login serves HTML):', result1 ? 'PASS' : 'FAIL');
    ...
    if (result1 && result2 && result3) {
      console.log('\n🎉 All tests passed! Admin routing is working correctly.');
    } else {
